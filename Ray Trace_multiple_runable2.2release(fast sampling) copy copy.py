...
if (tx>0 && ty>0 && tx<(float)resx && ty<(float)resy){
    ...
    int out_idx=pixel_idx*4+colorid;
    final_img[out_idx] = (unsigned char)scaled;
    //final_img[tid] = avg_color;
    if (colorid == 0) {
        final_img[pixel_idx * 4 + 3] = 255;
    }
}
'''
...
def generate_complex_test_chart(resy, resx):
    ...
    # 3. 绘制西门子星图 (Siemens Star - 核心分辨率测试)
    num_spokes = 36
    star_radius = 80
    for i in range(num_spokes):
        start_angle = i * (360 / num_spokes)
        end_angle = (i + 0.5) * (360 / num_spokes)
        pts = np.array([
            center,
            (center[0] + star_radius * np.cos(np.deg2rad(start_angle)), 
             center[1] + star_radius * np.sin(np.deg2rad(start_angle))),
            (center[0] + star_radius * np.cos(np.deg2rad(end_angle)), 
             center[1] + star_radius * np.sin(np.deg2rad(end_angle)))
        ], np.int32)
        cv2.fillPoly(chart, [pts], (1.0, 1.0, 1.0))

    # 4. 绘制四个角落的微缩文字 (测试边缘解析力)
    font = cv2.FONT_HERSHEY_SIMPLEX
    text_color = (0.8, 0.8, 0.8)
    # 左上角
    cv2.putText(chart, "RTX 5070", (20, 40), font, 0.7, text_color, 1)
    # 右下角 (极小字，测试 512 采样下的 AA)
...
cv2.imshow("Object", (img_cpu * 255).astype(np.uint8)) 
...
while not window.should_close() and running:
    t0 = time.time()
    ...
    #说明：前面的map_pbo传回一个 w * h * 4 字节的unsigned char数组，分别是R通道，G通道，B通道，灰度循环
    #C源码里面必须写成输出unsigned char否则内存泄露
    ...
    #     accumulated_buffer.fill(0)
    #     frames = 0
    #     print(f"Moving forward → z_len1 = {z_len1:.1f}")
    # elif key == ord('e'):
    #     z_len1 += z_step
    #     z_arr=cp.asarray([z_len1],dtype=np.float32)
    #     accumulated_buffer.fill(0)
    #     frames = 0
    #     print(f"Moving forward → z_Object = {z_len1:.1f}")
    # elif key == ord('E'):
    #     z_len1 += 5*z_step
    #     z_arr=cp.asarray([z_len1],dtype=np.float32)
    #     accumulated_buffer.fill(0)
    ...
    #     print("Exiting...")
    # reset = False
    # spd = 5*z_step if glfw.KEY_LEFT_SHIFT in window.key_pressed or glfw.KEY_RIGHT_SHIFT in window.key_pressed else z_step
    ...
    #     z_len1 -= spd; z_arr=cp.asarray([z_len1],dtype=np.float32); reset=True; print(f"z_len1 = {z_len1:.1f}")
    # if glfw.KEY_ESCAPE in window.key_pressed:
    #     running = False
    # if reset:
    ...
    t1 = time.time()
    dt = t1 - t0 + 1e-9
    print(f'Render time (z={z_Object:.1f}): {1/(t1-t0+0.0001):.1f}FPS')
